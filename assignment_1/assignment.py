...
class RatingsBreakdown(MRJob):
    ...
    def mapper_get_movies(self, _, line):
        ...
        yield movieID, 1 # we pass the movieId as the key, and it will receive initially 1 as a value

    def reducer_count_ratings(self, movieID, values):
        yield str(sum(values)).zfill(AMOUNT_OF_DECIMALS_FOR_RATINGS), movieID # turn the value into a string and fill the numbers with 0s to the left so it can be sorted, then pass the movieId, sum the values that share the movieId
    ...
    def reducer_sort_output(self, ratingsCount, movies):
        for movie in movies: # loop through the movieIds received in the previous reducer
            yield 'Movie ' + movie, ratingsCount + ' ratings.' # this code will 'print' each movie with the count of ratings ascendingly
